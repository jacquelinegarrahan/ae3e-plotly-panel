...
window.updateVariables = getLocationSrv().partial;

let templateSrv:any = getTemplateSrv();

interface Props extends PanelProps<SimpleOptions> {}

let updateRunId = function(data) {
  let variable = _.find(templateSrv.getVariables(), {'name':'run_id'});
  window.updateVariables({"var-run_id": data}, true);
  getLocationSrv().reload()
}
      ...
      display = (
        <Plot
          ...
          onSelected={(data) => {
            console.log("hey");
            console.log(data);
            let selectedpoints = data.points[0].fullData.selectedpoints;
            let selectedId = data.points[0].data.run_id[selectedpoints];
            console.log(data.points[0].data.run_id[selectedpoints]);
            //console.log(data);
            updateRunId(selectedId);
          }
          }
        ></Plot>
      );
